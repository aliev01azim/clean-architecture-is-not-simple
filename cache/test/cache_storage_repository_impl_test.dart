...
void main() {
  ...
  test('should get int result when saving articles to db.', () async {
    ...
    when(dbProvider.saveUser(randomContents, randomType))
        .thenAnswer((_) async => 1);

    final int result =
        await cacheStorageRepositoryImpl.saveUser(randomContents, randomType);
    expect(result, same(1));

    verify(dbProvider.saveUser(randomContents, randomType)).called(1);
  });
}
