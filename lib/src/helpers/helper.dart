...
const baseUrl = 'https://dasta.vilion-k.ru/api/v1';
...
Dio dio = Dio(
  BaseOptions(
    ...
    headers: {
      'authorization':
          'Bearer ${Hive.box('userBox').get('user', defaultValue: {})['accessToken']}',
    },
  ),
);
SnackBar bottomSnackBar(String text, [bool isInternet = false]) {
  return SnackBar(
    content: Text(text),
    duration: Duration(seconds: isInternet ? 5 : 3),
  );
}
...
void showInfo(context, GlobalKey key, text) async {
  ...
  final entry = OverlayEntry(
    builder: (_) => Positioned(
      ...
      child: Material(
        child: Container(
          color: const Color.fromARGB(255, 105, 110, 73),
